...
if  __name__ == "__main__":
    ...
    for fold, (train_idx, test_idx) in enumerate(kfold_cross_val.split(tweetSplits)):
        ...
        train_data = TweetGroupDataset(dataframe=train_df, samples = tweetSplits, tokenizer=tokenizer, textEncoder=textEncoder)
        val_data = TweetGroupDataset(dataframe=train_df, samples = tweetSplits, tokenizer=tokenizer, textEncoder=textEncoder)
        test_data = TweetGroupDataset(dataframe=train_df, samples = tweetSplits, tokenizer=tokenizer, textEncoder=textEncoder)
